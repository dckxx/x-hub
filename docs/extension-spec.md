...
6. `done` **service 托管**：进程托管（懒启动 / 动态端口 / TcpStream 探活 / 卸载清理 / 宿主退出停止）+ 运行时提供（复用系统 Node + 启动前校验 + 内置运行时按需下载 + 自动降级 + 设置项三选一：自动 / 始终内置 / 始终系统）+ 代理（桥 API `service.request` + `/svc/<extId>/*` HTTP 反向代理 + WebSocket 升级双向隧道，统一 CORS）。
