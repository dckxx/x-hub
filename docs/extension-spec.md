...
6. `in-progress` **service 托管**：已落地进程托管（懒启动 / 动态端口 / TcpStream 探活 / 卸载清理 / 宿主退出停止）+ 运行时提供（复用系统 Node + 启动前版本校验）+ 代理（桥 API `service.request` + `/svc/<extId>/*` HTTP 反向代理 + WebSocket 升级双向隧道，统一 CORS）。待补：按需下载内置运行时与自动降级（依赖扩展市场）。
