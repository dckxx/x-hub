...
6. `in-progress` **service 托管**：已落地进程托管（懒启动 / 动态端口 / TcpStream 探活 / 卸载清理 / 宿主退出停止）+ 运行时提供（复用系统 Node + 启动前版本校验）+ 非流式代理（桥 API `service.request` + `/svc/<extId>/*` HTTP 反向代理，统一 CORS）。待补：WebSocket 流式升级、按需下载内置运行时与自动降级（依赖扩展市场）。
7. `in-progress` 扩展中心补全：已落地扩展设置（信息/权限/卸载）+ 本地安装（选目录校验复制）+ 权限授权（运行时逐项开关，宿主侧 enforce）。待补：市场详情、在线安装页。
